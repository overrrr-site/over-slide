...
const FIELDS: Array<{ key: keyof BriefSheetData; label: string; rows: number }> = [
  ...
  { key: "reasoning_chain", label: "思考の流れ", rows: 3 },
  { key: "rejected_alternatives", label: "却下した選択肢", rows: 3 },
  { key: "key_expressions", label: "キーフレーズ", rows: 2 },
  { key: "discussion_note", label: "議論ノート", rows: 5 },
];
        ...
        <div className="mt-4 border-t border-beige pt-3">
          ...
          <div className="mt-2">
            <button
              ...
              className="w-full rounded-md bg-green px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-green/90 disabled:opacity-50"
            >
              {handingOff ? "作成中..." : "スライド資料として昇格する"}
            </button>
          </div>
